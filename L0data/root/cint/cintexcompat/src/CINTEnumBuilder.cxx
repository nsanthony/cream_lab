// @(#)root/cintex:$Id: CINTEnumBuilder.cxx 28764 2009-05-29 14:14:00Z pcanal $
...
void CINTEnumBuilder::Setup(const Type& e)
{
   ...
   if (!isCPPMacroEnum) {
      ...
      G__tagtable_setup(tagnum, sizeof(int), -1, 0, 0, 0, 0);
   }
   //--- setup enum values -------
   int isstatic;
   if (scope.IsTopScope()) {
   ...
   for (size_t i = 0; i < e.DataMemberSize(INHERITEDMEMBERS_NO); ++i) {
      ...
      string item = s.str();
      if (Cintex::Debug()) {
         cout << "Cintex:          item " << i << " " << item  << endl;
      }
      if (isCPPMacroEnum) {
         G__memvar_setup((void*) G__PVOID, 'p', 0, 0, -1, -1, -1, 1, item.c_str(), 1, 0);
      }
      else {
         G__memvar_setup((void*) G__PVOID, 'i', 0, 1, tagnum, -1, isstatic, 1, item.c_str(), 0, 0);
      }
   }
   if (scope.IsTopScope()) {
      G__resetglobalenv();
