 ...
 *
 ************************************************************************/
//$Id: rflx_gensrc.cxx 26743 2008-12-08 22:15:58Z pcanal $
...
// G__ci.h messes up fputc etc, #include after sys headers
...
void rflx_gensrc::gen_header()
{
   ...
   m_hd <<
       ...
       endl << "namespace Reflex { }" << std::
       endl <<
       "namespace seal { namespace reflex { using namespace Reflex; } }"
...
void rflx_gensrc::gen_datamemberdefs(G__ClassInfo & ci)
{
   ...
   while (dm.Next()) {
      if (strcmp("G__virtualinfo", dm.Name()) != 0
         ...
         ::Reflex::Member member = G__Dict::GetDict().GetDataMember((G__var_array*)dm.Handle(),dm.Index());
         G__getcomment(buf,&G__get_properties(member)->comment,G__get_tagnum(member.DeclaringScope()));
         if (buf[0]) {
...
void rflx_gensrc::gen_functionmemberdefs(G__ClassInfo & ci)
{
   ...
   while (fm.Next()) {
      ...
      if (fmname.length()) {
         ...
         ::Reflex::Member ift( G__Dict::GetDict().GetFunction((G__ifunc_table *) fm.Handle(),fm.Index()));

         if (ift.IsPublic()) {
            fm_modifiers = "PUBLIC";
         } else if (ift.IsProtected()) {
            fm_modifiers = "PROTECTED";
         } else if (ift.IsPrivate()) {
                                     ...
                                     G__ClassInfo & ci)
{
   ...
   s << ind() << "void* " << cldname 
      ...
      << ind() << "{" << std::endl;
   ++ind;
   s << ind() << "static std::vector<std::pair<Reflex::Base, int> > s_bases;" << std::endl;
   ...
   while (baseClassesToSearch.size()) {
      ...
      while (ciBase.Next()) {
         ...
         s << ind() << "s_bases.push_back(std::make_pair(Reflex::Base( Reflex::GetType< " 
            << basetype_name << " >(), Reflex::BaseOffset< " << fclname << "," << basetype_name 
...
void rflx_gensrc::gen_freefundicts()
{
   ...
   while (mi.Next()) {
      ...
      if ((fmname.length()) 
         ...
         std::string fun_ns = "";
         G__ClassInfo cl(G__get_tagnum(ifunc.DeclaringScope()));
