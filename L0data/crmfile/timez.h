 ...
 * hahaha
 *
 * Revision 1.2  2006/03/24 14:42:33  syzinn
 * Merged a branch post_flight2_changes
 *
 * Revision 1.1.1.1.4.1  2006/03/14 21:07:37  syzinn
 * eight-byte timestamp
 *
 * Revision 1.1.1.1  2005/02/04 05:02:17  daq
 * initial version for CREAM2.
 *
 * Revision 1.8  2004/10/15 20:52:14  syzinn
 * usec-resolution timestamping.
 *
 * Revision 1.7  2004/08/27 19:37:44  syzinn
 * added epoch_time() and epoch_time2() to get from time in secods since 2003/1/1.
 *
 * Revision 1.6  2004/05/20 18:23:38  syzinn
 * cputime() added.
 *
 * Revision 1.5  2004/04/15 21:04:45  syzinn
...
void fprint_cur_date_time(FILE* fp); /* current time */
void sprint_cur_date_time(char* s);

/*
 * Following functions are for timestamping.
 * Data structure for timestamp is as follows:
 *
 * typedef struct {
 *      unsigned padding: 2;
 *      unsigned nsec   :10;
 ...
 *	unsigned year	: 6; // year - 2000; up to 2063
 * } timestamp8_t;
 *
 * But the data structure is not exported.
 */
int timestamp(void* t);
time_t epoch_time(void* t);
time_t epoch_time2(unsigned* ta); // unsigned ta[6]
void filename_timestamp(char* s, void* t);
void fprint_timestamp(FILE* fp, void* t);
void sprint_timestamp(char* s, void* t);
unsigned long ComputeNumSecs(int,int,int,int,int,int);
